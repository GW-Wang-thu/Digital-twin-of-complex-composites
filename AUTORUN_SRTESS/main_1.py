...
class Sampler():
    ...
    def sample(self, mean_vect, sigma_vect, parser=None, decoder=None, force_label=None, tolerance=0.05):
        rec = []
        if parser is None:
...
def stress_predict_guassian(init_npys_dir, disp_force_list, save_dir, name_char="", layer="BM"):
    ...
    for train_ord in range(len(disp_force_list)):
        ...
        for j in range(len(disp_force_list)):
            ...
            if j == train_ord and key and disp_force_list[train_ord][1] > 16.0 and (disp_force_list[train_ord][0], disp_force_list[train_ord][1]) not in update_tuple:
            ...
            np.save(save_dir + str(j) + "\\" + str(train_ord) + "_" + name_char + layer + "_" + str(int(key)) + "_Bayesian_s11.npy", np.array(temp_s11, dtype='float32'))
            np.save(save_dir + str(j) + "\\" + str(train_ord) + "_" + name_char + layer + "_" + str(int(key)) + "_Bayesian_s22.npy", np.array(temp_s22, dtype='float32'))
            np.save(save_dir + str(j) + "\\" + str(train_ord) + "_" + name_char + layer + "_" + str(int(key)) + "_Bayesian_s33.npy", np.array(temp_s33, dtype='float32'))
            np.save(save_dir + str(j) + "\\" + str(train_ord) + "_" + name_char + layer + "_" + str(int(key)) + "_Bayesian_s12.npy", np.array(temp_s12, dtype='float32'))
            np.save(save_dir + str(j) + "\\" + str(train_ord) + "_" + name_char + layer + "_" + str(int(key)) + "_Bayesian_s13.npy", np.array(temp_s13, dtype='float32'))
            np.save(save_dir + str(j) + "\\" + str(train_ord) + "_" + name_char + layer + "_" + str(int(key)) + "_Bayesian_s23.npy", np.array(temp_s23, dtype='float32'))
...
if __name__ == '__main__':
    ...
    #                         save_dir=r"E:\Data\DATASET\SealDigitTwin\FINAL\STRESS\results_2\\",
    #                         name_char="20221021_",
