...
class success_coder(nn.Module):
    def __init__(self, vect_length, code_length):
        super(success_coder, self).__init__()

        self.encoder = nn.Sequential(
            nn.ReLU(),
            nn.Linear(vect_length, code_length),
        )

        self.decoder = nn.Sequential(
            nn.Linear(code_length, vect_length),
            nn.ReLU(),
        )
