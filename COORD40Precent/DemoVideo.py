...
class Coder_VAE():
    ...
    def decode_array(self, array):
        ...
        return np.array(x_rec, dtype='float32'), np.array(y_rec, dtype='float32'), np.array(z_rec, dtype='float32')
...
def coord_predict_guassian(init_npys_dir, add_npys_list, to_predict_list, name_char="", savedir="./results/", train_init=False, layer="BM"):
    ...
    my_Predictor.load_state_dict(torch.load("params/"+layer+"_Predictor_best.pth"))
    my_Predictor.eval()
    # Dataloader
    ...
    for i, [inputs, vects, _] in enumerate(my_add_Dataloader):
        ...
        for j in range(len(Codes_topre_Predict_Rec)):
            ...
            print("Decoding Time: ", time.perf_counter() - now)
            tmp_avg, tmp_std = temp_aftertrain_x[:, Concerning_Node].mean(), temp_aftertrain_x[:, Concerning_Node].std()
            avg_u.append(tmp_avg)
            std_u.append(tmp_std)
            real_u.append(label_u)
            tmp_avg, tmp_std = temp_aftertrain_y[:, Concerning_Node].mean(), temp_aftertrain_y[:, Concerning_Node].std()
            avg_v.append(tmp_avg)
            std_v.append(tmp_std)
            real_v.append(label_v)

        plot_bnd([avg_u, avg_v], [std_u, std_v], [real_u, real_v])
...
def plot_bnd(mean, std, label_rec=None, title=None):
    for m in range(len(mean)):
        mean_rec = mean[m]
        std_rec = std[m]
        plt.subplot(len(mean), 1, m+1)
        plt.plot(mean_rec, label="mean")
        if label_rec is not None:
            plt.plot(label_rec[m], label="label")
...
if __name__ == '__main__':
    ...
    topre_file_list = [update_list[40]]
    ...
    coord_predict_guassian(init_npys_dir=r"E:\Data\DATASET\SealDigitTwin\COORD\TRAIN\\",
                           ...
                           name_char="20221021_",
                           savedir=r"E:\Data\DATASET\SealDigitTwin\FINAL\COORD\results\L2\\",
                           train_init=False,
                           layer="BM")
