...
def train_autocoder(type="BM", save=False, display=False, step=10):
    trainset_dir = r"E:\Data\DATASET\SealDigitTwin\TransLearn\NPY\MergeTrain\\"
    validset_dir = r"E:\Data\DATASET\SealDigitTwin\TransLearn\NPY\MergeValid\\"
    ratio_train = True
    if ratio_train:
    ...
    if os.path.exists("./params_merge/" + type + "_coder_last.pth"):
        checkpoint = torch.load("./params_merge/" + type + "_coder_last.pth")
        autocoder.load_state_dict(checkpoint)
        results_rec = np.loadtxt("./params_merge/" + type + "_training_rec.txt")
    ...
    for i in range(epoch_n-len(results_rec)*step):
        ...
        if epoch % step == 0 or epoch == epoch_n - 1:
            ...
            if valid_loss_mean < min(loss_rec_all_valid):
                torch.save(autocoder.state_dict(), "./params_merge/" + type + "_coder_best.pth")
                np.savetxt("./params_merge/" + type + "_training_rec.txt", np.array(results_rec))
            loss_rec_all_valid.append(valid_loss_mean)
            print("Epoch %d, Train Loss: %.5f, Valid Loss: %.5f"%(epoch, train_loss_mean, valid_loss_mean))
            torch.save(autocoder.state_dict(), "./params_merge/" + type + "_coder_last.pth")
...
def train_predictor(type="BM", save=False, display=False, step=10):
    ...
    checkpoint = torch.load("./params_merge/" + type + "_coder_best.pth")
    autocoder.load_state_dict(checkpoint)
    autocoder.eval()

    # checkpoint = torch.load(type + "_Predictor_best.pth")
    # Code_predictor.load_state_dict(checkpoint)
    if os.path.exists("./params_merge/" + type + "_Predictor_last.pth"):
        checkpoint = torch.load("./params_merge/" + type + "_Predictor_best.pth")
        Code_predictor.load_state_dict(checkpoint)
        results_rec = np.loadtxt("./params_merge/" + type + "_predictor_training_rec.txt")
        loss_rec_all_valid = results_rec[:, 1].tolist()
        loss_rec_all_train = results_rec[:, 2].tolist()
        results_rec = results_rec.tolist()
    else:
    ...
    for i in range(epoch_n - len(results_rec) * step):
        ...
        if epoch % step == 0 or epoch == epoch_n - 1:
            ...
            if valid_loss_mean < min(loss_rec_all_valid):
                torch.save(Code_predictor.state_dict(), "./params_merge/" + type + "_predictor_best.pth")
                np.savetxt("./params_merge/" + type + "_predictor_training_rec.txt", np.array(results_rec))
            loss_rec_all_train.append(train_loss_mean)
            loss_rec_all_valid.append(valid_loss_mean)
            print("Epoch %d, Train Loss: %.5f, Valid Loss: %.5f" % (epoch, train_loss_mean, valid_loss_mean))
            torch.save(Code_predictor.state_dict(), "./params_merge/" + type + "_predictor_last.pth")
...
def eval_model(list_load, type, save=True):
    ...
    node_id = 1800
    stress_rec = []
    ratio_vect = np.array([4.5, 1.0, 4.5, 12.0, 25.0, 8.0])

    for i in range(len(list_load)):
        temp_input = torch.from_numpy(np.array(list_load[i], dtype="float32")).unsqueeze(0).cuda(0)
        outputs = Code_predictor(temp_input).unsqueeze(0)
        # print(inputs)
        shape_outputs = autocoder.decode(outputs)
        vect_outputs = shape_outputs[0, :].detach().cpu().numpy().T

        temp_node_stress = vect_outputs[node_id, :] / ratio_vect
        stress_rec.append(temp_node_stress)

    np.savetxt("list_stress.txt", np.array(stress_rec), delimiter='\t')
...
if __name__ == '__main__':
    ...
    # load_array_init = np.loadtxt(r'list_load.txt', delimiter='\t')
    # load_list = []
    # for i in range(load_array_init.shape[0]):
    #     load_list.append([load_array_init[i, 2], load_array_init[i, 3] - load_array_init[i, 4], load_array_init[i, 1]])
    # eval_model(list_load=load_list, type="BM")