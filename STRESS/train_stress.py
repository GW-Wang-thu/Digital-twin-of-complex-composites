...
class STRESS_dataloader(Dataset):
    ...
    def __getitem__(self, item):
        ...
        vect_torch_m = torch.transpose(torch.transpose(vect_torch, 0, 1) * torch.from_numpy(random_array).cuda() * self.ratio_vect, 0, 1)#
...
def train_autocoder(type="BM", save_code=False, display=False, step=10):
    ...
    if os.path.exists(params_dir + type + "_coder_last.pth"):
        checkpoint = torch.load(params_dir + type + "_coder_last.pth")
    ...
    for i in range(epoch_n-len(results_rec)*step):
        ...
        if epoch % step == 0 or epoch == epoch_n - 1:
            ...
            for i, (vects, file_name) in enumerate(valid_dl):
                ...
                if save_code:
                    code = mean[0].detach().cpu().numpy()
                    np.save(savedir_valid + file_name[0].split("\\")[-1], code)
...
def train_predictor(type="BM", save=False, display=False, step=10):
    ...
    return None
    checkpoint = torch.load("./params1/" + type + "_coder_last.pth")
    autocoder.load_state_dict(checkpoint)
    autocoder.eval()

    # checkpoint = torch.load(type + "_Predictor_best.pth")
    # Code_predictor.load_state_dict(checkpoint)
    if os.path.exists("./params1/" + type + "_Predictor_last.pth"):
        checkpoint = torch.load("./params1/" + type + "_Predictor_last.pth")
        Code_predictor.load_state_dict(checkpoint)
        results_rec = np.loadtxt("./params1/" + type + "_predictor_training_rec.txt")
    ...
    for i in range(epoch_n - len(results_rec) * step):
        ...
        if epoch % step == 0 or epoch == epoch_n - 1:
            ...
            if valid_loss_mean < min(loss_rec_all_valid):
                torch.save(Code_predictor.state_dict(), "./params1/" + type + "_predictor_best.pth")
            np.savetxt("./params1/" + type + "_predictor_training_rec.txt", np.array(results_rec))
            loss_rec_all_train.append(train_loss_mean)
            loss_rec_all_valid.append(valid_loss_mean)
            print("Epoch %d, Train Loss: %.5f, Valid Loss: %.5f" % (epoch, train_loss_mean, valid_loss_mean))
            torch.save(Code_predictor.state_dict(), "./params1/" + type + "_predictor_last.pth")
...
if __name__ == '__main__':
    # train_autocoder(type="BM", save_code=True, display=False, step=10)
    # train_autocoder(type="PR", save_code=True, display=False, step=10)
    # train_autocoder(type="NB", save_code=True, display=False, step=10)

    train_predictor(type="BM", save=False, display=True, step=10)
