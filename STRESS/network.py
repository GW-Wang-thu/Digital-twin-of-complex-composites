...
class conv_AUTOCODER(nn.Module):
    def __init__(self, vect_length, num_layer, code_length, in_channel=6):
        super(conv_AUTOCODER, self).__init__()
        self.in_channel = in_channel
        ds_fact_ds = (vect_length / code_length) ** (1 / num_layer)
        self.Conv_layer = nn.Sequential(
            nn.Conv1d(in_channels=self.in_channel, out_channels=12, kernel_size=1),
            nn.ReLU(),
        ...
        self.DeConv_layer = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=12, kernel_size=1),
            nn.ReLU(),
            nn.Conv1d(in_channels=12, out_channels=6, kernel_size=1),
        )
        print(self.Conv_layer, self.encoder, self.decoder, self.DeConv_layer)

    def forward(self, x, coder=0):
        if self.training:
            conv_code = self.Conv_layer(x)
            code = self.encoder(conv_code)
            decode = self.decoder(code)
            deconv_code = self.DeConv_layer(decode)
            return deconv_code
        else:
            if coder == 0:
                return self.encoder(self.Conv_layer(x))
            elif coder == 1:
                return self.DeConv_layer(self.decoder(x))
